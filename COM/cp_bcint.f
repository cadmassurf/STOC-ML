      subroutine cp_bcint(mx,my,mz,ldno,ndt,dat)
c
c [global entities]
...
c
c [intent(inout)]
      Integer   DAT(1-LX:MX-2+LX,1-LY:MY-2+LY,1-LZ:MZ-2+LZ,NDT)
ccc      Integer      dat(1-lx:mx+lx,1-ly:my+ly,1-lz:mz+lz,ndt)
c
c [local entities]
      integer  itag
      parameter( itag=0 )
      ...
      integer   sbuf(mcmsbf*ndt),rbuf(mcmrbf*ndt)
...
c
      ...
      if( isbf(np).gt.0 ) then
        ...
        ne=ne+nko
        nqs=nqs+1
        call mpi_isend(sbuf(ns),nko,mpi_integer,np,itag
...
c
      ...
      if( irbf(np).gt.0 ) then
        ...
        ne=ne+nko
        nqr=nqr+1
        call mpi_irecv(rbuf(ns),nko,mpi_integer,np,itag
