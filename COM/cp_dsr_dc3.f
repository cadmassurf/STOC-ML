      subroutine cp_dsr_dc3(MM,mx,my,mz,ldno,ndt,dat)
c
c [global entities]
...
c
c [intent(inout)]
      REAL*8   DAT(MM,1-LX:MX-2+LX,1-LY:MY-2+LY,1-LZ:MZ-2+LZ,NDT)
...
c [local entities]
      ...
      real*8   sbuf(mcmsbf*ndt*mm),rbuf(mcmrbf*ndt*mm)
      integer  i,j,k,l,m,ld,ldn,ierr
...
c
      ...
      do 100 nc=1,ncms
        ...
        do 110 m=1,mm
          ne=ne+1
          sbuf(ne)=dat(m,i,j,k,l)
  110   continue
        np=indcms(0,nc)
        if( np.lt.npb ) then
          CALL ERRMSG('CP_DSR_DC3',6240)
...
c
      ...
      do 300 nc=1,ncmr
        ...
        do 310 m=1,mm
          ne=ne+1
          dat(m,i,j,k,l)=rbuf(ne)
  310   continue
        np=indcmr(0,nc)
        if( np.lt.npb ) then
          CALL ERRMSG('CP_DSR_DC3',6241)
