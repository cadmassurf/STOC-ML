      SUBROUTINE CLHUVW(HU,HV,HW,UU,VV,WW,FF,GV,GX,GY,GZ,GV0,GX0,GY0,
...
C
     ...
     $                +         FF2*MIN(UU(I,J,K),0.0D0))
            HU(I,J,K) = HU(I,J,K)*GX(I,J,K)/GX0(I,J,K)
         END IF
C
C ...... 線流量を合計しておく(ワークエリア)
...
C
     ...
     $                +         FF2*MIN(VV(I,J,K),0.0D0))
            HV(I,J,K) = HV(I,J,K)*GY(I,J,K)/GY0(I,J,K)
     $                *YCOSP(J)
         END IF
C
C ...... 線流量を合計しておく(ワークエリア)
