      SUBROUTINE BOUND(HU,HV,FF,PP,RHOW,HH,HOLD,HDEP,XC,YC,ZC,
...
C
      ...
      REAL(8)::SA,SB,UUU,VVV,XX,YY,ZZZ
ckt ---
      REAL(8)::depth,fw,fe,fs,fn,ztdmm,HH2,RGD,MN,HHOLD
      INTEGER::IH1,IH2,L,N
ckt ---
CCC      real(8):: lat1,lon1,lat,lon,dir,lon2,lat2
CCC      real(8),parameter::PI = 3.141592653897932d0
c
...
ctest            if( ICORDTYPE.eq.2.and.j.eq.2.or.j.eq.mym ) then
ctest            lon1=-0.5d0*(xgrid(i-1)+xgrid(i))*PI/180.d0
ctest            lat1=0.5d0*(ygrid(j-1)+ygrid(j))*PI/180.d0
ctest            dir=atan2(dble(ii-i)*uuu,dble(jj-j)*vvv)
ctest            lat2=asin(sin(lat1)*cos(CC/REARTH)
ctest     $               +cos(lat1)*sin(CC/REARTH)*cos(dir))
ctest            lon2=mod(lon1-asin(sin(dir)*sin(CC/REARTH)/cos(lat2))
ctest     $          +PI,2.d0*PI)-PI
ctest            lon1=-lon1
ctest            lon2=-lon2
ctest            XX=abs(lon2-lon1)*REARTH*cos(0.5d0*(lat1+lat2))
ctest            YY=abs(lat2-lat1)*REARTH
ctestc            write(*,*) 'XX:',XX,DX,lon1*180.d0/PI,lon2*180.d0/PI
ctestc            write(*,*) 'YY:',YY,DY,lat1*180.d0/PI,lat2*180.d0/PI
ctest            else
